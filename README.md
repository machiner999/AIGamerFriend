...
- AIの感情を反映するアニメーション顔表示（7種類: NEUTRAL, HAPPY, EXCITED, SURPRISED, THINKING, WORRIED, SAD）
  - 感情ごとのアクセントカラー（緑/金/シアン/紫/橙/青）で表情が変化
  - 3層ラジアルグローによるブルームエフェクト
  - オーディオリアクティブ破線リング（音声レベルに応じてリング表示が変化）
  - まばたき・目線移動のアイドルアニメーション
  - 感情変化時のパーティクルバーストエフェクト
- Function Callingによるゲーム名自動検出・表示（スライドインアニメーション付き）
...
- ミュート機能・8本バーの音声レベルインジケーター（感情カラーグラデーション対応）
- マイク音声のソフトウェアゲイン増幅（デフォルト2倍）
- 設定画面（声の種類・リアクション強度・自動スタート・記憶クリア）— セクション別グラスカードUI
- 初回オンボーディング画面（AIFaceプレビュー付き）
- 触覚フィードバック（接続/エラー/感情変化時）
- グラスモーフィズムUI全面適用（API 31+でリアルぼかし、フォールバックあり）
- 状態遷移アニメーション（接続中パルスリング、エラーシェイク、ステータスのスライド遷移）
...
依存関係の脆弱性スキャン用に GitHub Actions の `Snyk` ワークフローを追加しています。`main` への push、`main` 向け PR、手動実行で動作します。

1. [Snyk](https://snyk.io/) でアカウントを作成し、API トークンを取得
2. GitHub リポジトリの `Settings > Secrets and variables > Actions` で `SNYK_TOKEN` という Repository secret を追加
3. 必要に応じて Actions タブから `Snyk` ワークフローを手動実行

このワークフローは Android ビルドに必要な `google-services.json` と `local.properties` を CI 用のダミー値で生成した上で、Gradle 依存関係を Snyk でスキャンします。結果は GitHub Security タブへ SARIF としてアップロードされ、高 severity 以上の脆弱性が見つかるとジョブを失敗させます。
...
├── ui/
│   ├── screen/
│   │   └── GamerScreen.kt          # メイン画面 (カメラ + オーバーレイ + 状態遷移アニメ)
│   ├── component/
│   │   ├── AIFace.kt               # 感情アニメーション顔 (Canvas描画, 感情別カラー, パーティクル)
│   │   ├── CameraPreview.kt        # CameraX連携・フレームキャプチャ
│   │   ├── StatusOverlay.kt        # 接続状態インジケータ (グラスモーフィズム, シェイク)
│   │   ├── GlassControlPanel.kt    # コントロールパネル (ボタン, 音声ビジュアライザー)
│   │   ├── GlassModifier.kt        # 再利用可能グラスモーフィズムModifier
│   │   ├── OnboardingOverlay.kt    # 初回オンボーディング (AIFaceプレビュー付き)
│   │   ├── SettingsSheet.kt        # 設定ボトムシート (セクション別カード)
│   │   └── HelpSheet.kt            # 使い方ガイドボトムシート
│   └── theme/
│       ├── Color.kt                # カラーパレット (感情別アクセント, ガラストークン, ステータス)
│       ├── Type.kt                 # タイポグラフィ
│       └── Theme.kt                # Material 3 ダークテーマ
└── util/
