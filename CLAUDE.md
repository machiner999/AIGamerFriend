...
デバイスの背面カメラでゲーム画面を見て、カジュアルなゲーム友達として音声でリアクションするAndroidアプリ。OkHttp WebSocketでGemini Multimodal Live APIに直接接続し、リアルタイム双方向の音声+映像ストリーミングを行う。Function Calling（感情関数 + Google検索 + ゲーム名検出 + 音声コマンド）を通じてAIの感情を反映するアニメーション顔を表示する。Firebase AI Logic SDKは要約生成（non-live `GenerativeModel`）にのみ使用。ミュート機能、音声レベルインジケーター、初回オンボーディング、ゲーム名表示、音声コマンド（セッション終了・ミュート切替）、自動スタート、設定画面（声・リアクション強度・自動スタート・記憶クリア）を搭載。
...
**UIレイアウト**: フルスクリーンカメラプレビューにオーバーレイを重ねる構成。UIコンポーネントは `ui/component/` 配下に分割。`StatusOverlay`（左上、`glassPanel()` + `shadow` + エラー時シェイクアニメーション）が接続状態を表示し、その下にゲーム名ラベル（NeonBlueグラデ背景 + `SportsEsports`アイコン + `slideInHorizontally`）を表示。`AIFace`（下部中央、コントロールの上）がConnected時に `fadeIn + scaleIn(spring)` で登場。接続中は同位置にパルスする破線リングを表示。`GlassControlPanel`（`ui/component/GlassControlPanel.kt`、下部固定）にステータステキスト（`AnimatedContent` + `slideInVertically`）＋8本バー音声レベルインジケーター（感情カラーグラデーション + グロー）＋ミュートボタン（赤ティント背景 + スケールアニメ）＋セッションボタン（アイコン＋テキスト + 接続中パルスリング + プレスアニメ）＋設定ボタンを配置。`WindowInsets.safeDrawing` に対応。全オーバーレイに `Modifier.glassPanel()` を適用（`ui/component/GlassModifier.kt`: グラデーションボーダー、上端ハイライト、API 31+で`RenderEffect.createBlurEffect`によるリアルぼかし）。初回起動時は `OnboardingOverlay`（`ui/component/OnboardingOverlay.kt`: AIFaceが感情を切り替えるプレビュー + パルスする「はじめる」ボタン）が表示される。設定は `SettingsBottomSheet`（`ui/component/SettingsSheet.kt`: セクション別 `glassPanel` カード）、ヘルプは `HelpBottomSheet`（`ui/component/HelpSheet.kt`）に分離。

**デザインシステム** (`ui/theme/`): `Color.kt` に感情別アクセントカラー（`EmotionGreen`/`EmotionGold`/`EmotionCyan`/`EmotionPurple`/`EmotionOrange`/`EmotionBlue`）、ガラスエフェクトトークン（`GlassBg`/`GlassBorder`/`GlassHighlight`）、ステータスカラー（`StatusLive`/`StatusWarning`/`StatusError`/`StatusDelayed`）を定義。`Emotion.accentColor()` 拡張関数で感情→カラーのマッピング。`Type.kt` に `bodyMedium`/`bodySmall`/`titleMedium` を明示定義、`labelSmall` は `letterSpacing = 1.5.sp` でHUD感を演出。

**感情システム + ゲーム名検出 + 音声コマンド**: Geminiがパラメータなし関数 `setEmotion_HAPPY`、`setEmotion_SAD` 等をFunction Callingで呼び出し、AIの表情を更新する。加えて `setGameName(name: String)` でプレイ中のゲーム名を検出・表示する。音声コマンドとして `stopSession`（「終わり」「やめる」「ストップ」等）と `toggleMute`（「ミュート」「黙って」「静かにして」等）もFunction Callingで実装。`stopSession` は `viewModelScope.launch` で非同期実行する（WebSocketコールバック内からの同期呼び出しはリエントラント問題を起こすため）。`toolDeclarations`（感情ごとに1つの `FunctionDeclaration` + `setGameName` + `stopSession` + `toggleMute`）として定義。`GamerViewModel` の `handleFunctionCall(name, callId, args)` が関数名から処理を分岐し、感情名を抽出して `_currentEmotion` を更新、ゲーム名を `_gameName` に設定、またはセッション停止/ミュート切替を実行後、`sendToolResponse` で結果を返送。ミュート解除はGemini経由では不可（ミュート中は音声がGeminiに送信されないため）。`AIFace` コンポーザブル（180dp）がスプリング物理でステート間をアニメーションする。Canvasに感情別アクセントカラー（`Emotion.accentColor()` + `animateColorAsState`）の目/眉/口を半透明黒円の上に描画。3層ラジアルグロー（外側薄い→内側濃い）でブルームエフェクト。オーディオリアクティブ破線リング（`drawCircle` + `PathEffect.dashPathEffect`、常時回転 + 音声レベルでalpha/ダッシュ間隔変化）。まばたき（3-5秒周期で `eyeRadiusY` を一瞬0に近づける `Animatable`）。目線ドリフト（5秒周期 `keyframes` でX方向微移動）。感情変化時にパーティクルバースト（10個の小円、感情カラーで着色、外側に放射しながらフェードアウト ~500ms）。呼吸アニメーション（`rememberInfiniteTransition` による微妙なスケール振動）と感情変化時のバウンスエフェクト（`Animatable` スプリング）。`audioLevel: Float = 0f` パラメータ追加（デフォルト値でAPI互換）。`FaceParams`/`paramsFor()` は変更なし（テスト影響ゼロ）。Connected時のみ表示。7つの感情: NEUTRAL, HAPPY, EXCITED, SURPRISED, THINKING, WORRIED, SAD。
...
- `AudioManager.kt` — マイク録音（`VOICE_COMMUNICATION`ソース、16kHz mono PCM 16bit、チャンク2048B、`Dispatchers.IO`）とスピーカー再生（24kHz mono PCM 16bit、バッファ4倍）。`AcousticEchoCanceler`（AEC）、AGC、NoiseSuppressorをハードウェアレベルで有効化。`isMuted` フラグでマイク送信を抑制（録音自体は継続）。`onAudioLevelUpdate` コールバックでRMS音声レベル（0.0-1.0）を通知。`gainFactor`（デフォルト10.0f）でPCM 16bitサンプルをソフトウェア増幅し、クリッピング対応済み（~1m距離での使用を想定）。NoiseSuppressorは無効（遠距離の弱い音声をノイズとして誤抑制するため）。増幅はAGC/AECの後段で適用される。
- `SettingsStore.kt` — DataStore Preferencesによるアプリ設定永続化（オンボーディングフラグ、声の種類、リアクション強度、自動スタート）。
...
**ツールはOkHttp WebSocket直接接続で有効**: 感情関数（`setEmotion_*`、`parameters = null`）、ゲーム名検出（`setGameName`、`parameters` = OBJECT with `name: STRING`）、音声コマンド（`stopSession`・`toggleMute`、`parameters = null`）、Google検索（`"google_search": {}`）を `tools` リストに格納し、セットアップメッセージで送信。firebase-aiはBOM管理（要約モデル用のみ使用、Live APIはOkHttp直接接続のためピン不要）。
...
テストファイル: `GamerViewModelTest.kt`（セッションライフサイクル、リトライ、感情処理、resumeトークン、ミュート、ゲーム名、音声コマンド、オンボーディング）、`GeminiLiveModelsTest.kt`（シリアライズ検証 — `parameters` vs `parameters_json_schema`、圧縮・セッション再開・GoAway、デシリアライズ）、`GeminiLiveClientTest.kt`（接続状態遷移、空APIキー、resumeトークン初期値）、`GamerScreenKtTest.kt`（触覚ロジック、状態ヘルパー）、`AIFaceKtTest.kt`（感情パラメータマッピング）、`StatusOverlayKtTest.kt`（オーバーレイ状態マッピング — `StatusLive`/`StatusWarning`/`StatusError`/`StatusDelayed` 定数を使用）、`CameraPreviewKtTest.kt`（フレームスロットルタイミング）、`EmotionTest.kt`（enumパース）、`PermissionHelperTest.kt`（パーミッションチェック）、`MemoryStoreTest.kt`（追加/取得/クリア/容量制限/切り詰め）、`SettingsStoreTest.kt`（オンボーディングフラグ、声・リアクション強度・自動スタートの読み書き）。

いくつかのソース関数は `@VisibleForTesting internal` マークされており、本来privateなロジックのユニットテストを可能にしている（例: AIFaceの `paramsFor()`、StatusOverlayの `statusOverlayInfo()`、CameraPreviewの `shouldCaptureFrame()` / `downscaleBitmap()`、GamerScreenの `isSessionActive()` / `hapticForSessionTransition()` / `hapticForEmotionChange()` / `HapticType`）。UIコンポーネント分割（`GlassControlPanel.kt`、`OnboardingOverlay.kt`、`SettingsSheet.kt`、`HelpSheet.kt`）後もテスト対象の `@VisibleForTesting` 関数は `GamerScreen.kt` と `AIFace.kt` に残留しており、テストへの影響はない。`testDebugUnitTest` のみ使用 — releaseユニットテストバリアントは存在しない。
